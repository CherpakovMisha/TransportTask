...
void Table::methodDoubleAproximate()
{
	if (err != ERR_OK)
		return;
	...
	while (check != width + height - 1)//one iteration of the algorithm
	{
		if (mycheck == true)
		{
			...
			for (i = 1; i < height; ++i)
			{
				int min_in_line = table[i][j]._data.price;
				for (j = 1; j < width; ++j)
				{
					if (table[i][j]._data.price < min_in_line)
						min_in_line = table[i][j]._data.price;
				}
				for (j = 1; j < width; ++j)
				{
					if (table[i][j]._data.price == min_in_line)
					{
						if (table[i][j].used1 == true) {
							table[i][j].used2 = true;
							break;
						}

						else {
							table[i][j].used1 = true;
							break;
						}
			...
			for (j = 1; j < width; ++j)
			{
				int min_in_collum = table[i][j]._data.price;
				for (i = 1; i < height; ++i)
				{
					if (table[i][j]._data.price < min_in_collum)
						min_in_collum = table[i][j]._data.price;
				}
				for (i = 1; i < height; ++i)
				{
					if (table[i][j]._data.price == min_in_collum)
...
void Table::saveSolution()
{
	...
	{		
		...
		Solution += Cargo* table[i][j]._data.price;
...
std::ostream& operator << (std::ostream &out, Table &table)
{
	...
	for (unsigned int j = 1; j < table.height; j++)
	{
		for (unsigned int i = 0; i < table.width; i++)
		{
			...
			if (i != 0)
				out << "/" << table.table[j][i].value << "/" << ((table.table[j][i].used1) ? "T" : "F") << "/" << ((table.table[j][i].used2) ? "T" : "F") << " ";
			out << " ";
		}
		out << std::endl;
	}
...
std::pair<int, int> Table::findNewStartPoint()
{
	...
	while (stack.size() != 0)
	{
		...
		for (int i = 1; i < height; i++)
		{
			if (!table[i][a.second].used1) continue; //если клетка не юазалась то идЄм дальше
		...
		for (int j = 1; j < width; j++)
		{
			if (!table[a.first][j].used1) continue; //если клетка не юазалась то идЄм дальше
...
bool Table::is_useable(std::pair<int, int> point, std::list<std::pair<int, int>>& chain) {
	if (!table[point.first][point.second].used1)
		return false;
	...
	while (true)
	{
		...
		if (point.first == -1) {
			...
			for (int i = 0; i < height; ++i) 
			{
				...
				{
					if (table[i][j].used1 = true)
						Solution += table[i][j]._data.price*table[i][j].value;
				}
			}
